...
for (let i = 0; i < lines.length; i++) {
  ...
  // 检测URL行
  else if (currentSource && line.match(/^https?:\/\//)) {
    currentSource.url = line.split(' ')[0]; // 取第一个URL
  }
  // 收集摘要信息（排除特定格式的行）
  else if (currentSource && line && 
           !line.startsWith('---') && 
           !line.startsWith('##') && 
           !line.startsWith('*') && 
           !line.match(/^https?:\/\//) &&
           !line.includes('### Image') &&
           !line.includes('Now Playing') &&
           !line.includes('Heraskevych')) {
...
sources.forEach(source => {
  ...
  if (source.summary.length > 500) {
    source.summary = source.summary.substring(0, 500) + '...';
  }
});
...
sources.slice(0, 3).forEach((source, index) => {
  ...
  console.log(`   摘要: ${source.summary.substring(0, 100)}...`);
  console.log('');
});